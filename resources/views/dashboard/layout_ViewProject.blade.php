...
<script>
    ...
    $("#project_files2").fileinput({
        ...
        initialPreview: [
            @foreach($project->approvedDocs as $document)
                "{{ asset('storage/'.$document->file) }}",
            @endforeach
        ],
        initialPreviewConfig: [
                @foreach($project->approvedDocs as $document)
            {caption: "{{ $document->file }}", width: "120px", url: "{{ route('dashboard.projects.approved.remove', $document->id) }}", key: "{{ $document->id }}"},
            @endforeach
        ],
        allowedFileExtensions: ['jpg', 'png', 'gif', 'jpeg','pdf', 'doc', 'docx', 'xls', 'xlsx']
    });
